"""
Small CNN wrappers for eye and mouth ROI classification.

Each wrapper loads a trained checkpoint, applies the same preprocessing used
during training, and returns the target class probability for a BGR crop.
"""
...
from fatigue_pipeline.constants import EYE_CLASS_CLOSED, EYE_MODEL_INPUT_HW, IMAGENET_MEAN, IMAGENET_STD, MOUTH_CLASS_OPEN, MOUTH_MODEL_INPUT_HW
...
def _resolve_checkpoint(path_or_spec: str | Path) -> Path:
    ...
    if isinstance(path_or_spec, str) and path_or_spec.startswith("hf://"):
        ...
        # Format: hf://<user_or_org>/<repo>/<filename>
...
class _BinaryROIPredictor:
    """
    Shared loader and preprocessor for binary ROI classifiers
    """
    ...
    def predict_prob(self, crop_bgr: np.ndarray | None) -> float | None:
        """
        Return the target class probability for one crop
        """
    ...
    def predict_probs_batch(
        ...
        """
        Run prediction on a batch of crops, keeping None for missing crops
        """
